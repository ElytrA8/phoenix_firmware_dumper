...
if [[ -s "${PROJECT_DIR}"/.github_token ]]; then
	...
	git commit -q -asm "Add ${description}"
	git push https://${GITHUB_TOKEN}@github.com/${GIT_ORG}/${repo}.git "${branch}" || (
		git update-ref -d HEAD
		git reset system/ vendor/
		git checkout -b "${branch}"
		git commit -q -asm "Add extras for ${description}"
		git push https://${GITHUB_TOKEN}@github.com/${GIT_ORG}/${repo}.git "${branch}"
		git add vendor/
		git commit -q -asm "Add vendor for ${description}"
		git push https://${GITHUB_TOKEN}@github.com/${GIT_ORG}/${repo}.git "${branch}"
		git add system/system/app/ system/system/priv-app/ || git add system/app/ system/priv-app/
		git commit -q -asm "Add apps for ${description}"
		git push https://${GITHUB_TOKEN}@github.com/${GIT_ORG}/${repo}.git "${branch}"
		git add system/
		git commit -q -asm "Add system for ${description}"
		git push https://${GITHUB_TOKEN}@github.com/${GIT_ORG}/${repo}.git "${branch}"
	)
